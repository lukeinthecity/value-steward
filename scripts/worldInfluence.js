...
function summarize(intents) {
  ...
  for (const intent of intents) {
    if (intent.reason_code === "WORLD_CONTEXT_STALE") counters.world_context_stale += 1;
    ...
    if (intent.gate_macro_buy_allowed === false) counters.macro_buy_blocked += 1;
    if (intent.gate_macro_sell_allowed === false) counters.macro_sell_blocked += 1;
  }
