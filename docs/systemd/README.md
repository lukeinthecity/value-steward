...
- `value-steward-tick.service` + `value-steward-tick.timer`: execution scheduler for the pre-close tick slots.
- `value-steward-world.service` + `value-steward-world.timer`: scheduled world context runner.
- `value-steward-world-health.service` + `value-steward-world-health.timer`: world health/context refresh outside market hours.
...
4. Enable and start:
   ...
   - `sudo systemctl enable --now value-steward-world-health.timer`
   - `sudo systemctl enable --now value-steward-eod.timer`

Notes:
- Timers use `Timezone=America/New_York` so they fire on market time.
- The world/tick/eod services call the scheduled wrappers:
  - `npm run world:run:scheduled`
  - `npm run world:health:scheduled`
  - `npm run local:tick:scheduled`
  - `npm run eod:run:scheduled`
  These scripts guard against off-window execution and duplicate runs.
- `value-steward-world-health.timer` runs at `06:05` and `18:05` New York time every day,
  including weekends and holidays, so `world-health.json` and weekend macro context do not go stale.
