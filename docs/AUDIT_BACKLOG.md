...
| `_parse_hhmm()` for early-close time | `execution_engine.py` | ✅ | Applied (PR after #45). **Correction:** earlier marked ⏸ by mistake — I'd checked `_market_close_time` (already used `_parse_hhmm`) but `b32a76f` actually fixed `_is_market_open_now`, which still had a crash-prone `.split(":")`/`int()` parse on malformed holiday `close_time`. Now routed through `_parse_hhmm`; 8 parser tests. |
