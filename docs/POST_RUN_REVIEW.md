...
| 15 | Platform feature parity (short selling, German equities, index options) | **OVERRULED for Run 4 — deferred to Run 5.** The original wording ("pick at most one new asset class to prototype for Run 4") predates the clean-baseline decision and directly contradicts it; ML_BACKLOG 3.5 itself says such a change warrants its own dedicated run. Revisit at the Run-4 post-run review, lowest-lift first (German equities < short selling < options), and re-check Alpaca's changelog first — that list goes stale fast | ML_BACKLOG 3.5 |
...
| 17 | **Fix the OOS measurement faults — do this FIRST** | Always, and before any other item: sign/population fix for declined rows, and de-duplication by `(symbol, exchange_date, horizon)`. Every conclusion below depends on these; running the rest first means judging mechanisms with a broken ruler | ML_BACKLOG known limitations |
| 18 | Complexity audit — prune mechanisms the evidence can't carry | Always, *after* item 17. Bias toward removal. Order: `signal_weights_by_regime` (idle 17/20 cycles) → the three undocumented score nudges (exec-quality 10%, alpha prior 10%, intraday 5%) → pattern bias (gated on 2.2) → redundant gates (3.4) | ML_BACKLOG 3.7 |
| 19 | Execution policy (2.8) — **premise refuted, do not act on the KALV story** | Measured 2026-08-06: high-conviction fills *best* (60% vs 40% mid, 20% low). Re-measure at larger n; only revisit if the gradient inverts | ML_BACKLOG 2.8 |
| 20 | **Fix `scorecard-summary.json` BEFORE using §1.1's cross-check** | Its `avg_excess_benchmark` is unfiltered and replica-weighted, so the independent check on the verdict is currently as broken as the primary metric was pre-#115. Either fix it or drop it from §1.1 — do not cross-check against it as-is | ML_BACKLOG 3.9 |
| 21 | Correlation matrix top-slice (unlocks the real universe) | The recommended **first** post-baseline experiment. O(n²) over the full list caps the universe at ~800; a top-slice projects 6,535 symbols in under a minute. Decision-affecting (feeds the diversification gate), so verify that gate is unchanged on retained pairs | ML_BACKLOG 3.8 |
| 22 | Realistic test fixtures — **do this before re-enabling any learning flag** | Prevents recurrence rather than fixing an instance. Suites model BUY-only populations that have never existed live (~73–80% is `NO_ACTION`), which is why one fault class survived in four modules at once | ML_BACKLOG 3.11 |
...
- **2026-08-06** — Added item 16 (`ML_BACKLOG.md` 3.6, user-directed
  thesis testing) from a Day-19 check-in conversation: the user wants to
  state a market thesis in plain language and have it tested against
