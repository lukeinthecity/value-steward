import { sendLessonEmail } from "./emailNotifications.js";
...
const WORLD_CONTEXT_PATH = "data/world-context.jsonl";
...
async function loadWorldContextFromGitHub(githubToken) {
  return loadLatestWorldContext({ githubToken });
}
...
async function loadAgentState(token) {
  ...
  if (!res.ok) {
    throw new Error(
      `Error loading agent state: ${res.status} ${await res.text()}`
    );
  }
